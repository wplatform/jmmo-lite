...
public class SpellHandler {
    ...
    void HandleUseItemOpcode(WorldPackets::Spells::UseItem& packet)
    {
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR("network", "HandleUseItemOpcode: attempted to cast a non-existing spell (Id: {})", packet.Cast.SpellID);
            return;
        }
    ...
    void HandleCastSpellOpcode(WorldPackets::Spells::CastSpell& cast)
    {
        // Skip casting invalid spells right away
        SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(cast.Cast.SpellID, _player->GetMap()->GetDifficultyID());
        if (!spellInfo)
        {
            TC_LOG_ERROR("network", "HandleCastSpellOpcode: attempted to cast a non-existing spell (Id: {})", cast.Cast.SpellID);
            return;
        }
