...
public class InspectHandler {
    ...
    void HandleInspectOpcode(WorldPackets::Inspect::Inspect& inspect)
    {
        ...
        if (!player)
        {
            TC_LOG_DEBUG("network", "HandleInspectOpcode: Target {} not found.", inspect.Target.ToString());
            return;
        }

        TC_LOG_DEBUG("network", "HandleInspectOpcode: Target {}.", inspect.Target.ToString());
    ...
    void HandleQueryInspectAchievements(WorldPackets::Inspect::QueryInspectAchievements& inspect)
    {
        ...
        if (!player)
        {
            TC_LOG_DEBUG("network", "HandleQueryInspectAchievements: [{}] inspected unknown Player [{}]", GetPlayer()->GetGUID().ToString(), inspect.Guid.ToString());
            return;
        }

        TC_LOG_DEBUG("network", "HandleQueryInspectAchievements: [{}] inspected Player [{}]", GetPlayer()->GetGUID().ToString(), inspect.Guid.ToString());
