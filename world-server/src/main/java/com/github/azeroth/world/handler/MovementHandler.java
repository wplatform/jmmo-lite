...
public class MovementHandler {
    ...
    void HandleMovementOpcode(OpcodeClient opcode, MovementInfo& movementInfo)
    {
        ...
        // ignore, waiting processing in HandleMoveWorldportAckOpcode and HandleMoveTeleportAck
    ...
    void HandleForceSpeedChangeAck(WorldPackets::Movement::MovementSpeedAck& packet)
    {
        ...
        switch (opcode)
        {
            ...
            default:
                TC_LOG_ERROR("network", "HandleForceSpeedChangeAck: Unknown move type opcode: {}", opcode);
                return;
        }
        ...
        if (!_player->GetTransport() && std::fabs(_player->GetSpeed(move_type) - packet.Speed) > 0.01f)
        {
            ...
            else                                                // must be lesser - cheating
            {
                ...
                _player->GetSession()->KickPlayer("HandleForceSpeedChangeAck Incorrect speed");
            }
        }
    }
        ...
        if (_player->m_movementForceModMagnitudeChanges > 0)
        {
            ...
            if (!_player->m_movementForceModMagnitudeChanges)
            {
                ...
                if (std::fabs(expectedModMagnitude - setModMovementForceMagnitudeAck.Speed) > 0.01f)
                {
                    ...
                    _player->GetSession()->KickPlayer("HandleMoveSetModMovementForceMagnitudeAck Incorrect magnitude");
                    return;
                }
            }
        }
    ...
    void HandleMoveTimeSkippedOpcode(WorldPackets::Movement::MoveTimeSkipped& moveTimeSkipped)
    {
        ...
        if (!mover)
        {
            TC_LOG_WARN("entities.player", "HandleMoveTimeSkippedOpcode wrong mover state from the unit moved by {}", GetPlayer()->GetGUID().ToString());
            return;
        }
        ...
        {
            TC_LOG_WARN("entities.player", "HandleMoveTimeSkippedOpcode wrong guid from the unit moved by {}", GetPlayer()->GetGUID().ToString());
            return;
        }
