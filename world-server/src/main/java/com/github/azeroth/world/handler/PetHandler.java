...
public class PetHandler {
    ...
    void HandlePetActionHelper(Unit* pet, ObjectGuid guid1, uint32 spellid, uint16 flag, ObjectGuid guid2, Position const& pos)
    {
        ...
        if (!charmInfo)
        {
            TC_LOG_DEBUG("entities.pet", "HandlePetAction(petGuid: {}, tagGuid: {}, spellId: {}, flag: {}): object {} is considered pet-like but doesn't have a charminfo!",
    ...
    void HandlePetSetAction(WorldPackets::Pet::PetSetAction& packet)
    {
        ...
        if (!charmInfo)
        {
            TC_LOG_ERROR("entities.pet", "HandlePetSetAction: object {} is considered pet-like but doesn't have a charminfo!", pet->GetGUID().ToString());
            return;
        }
    ...
    void HandlePetSpellAutocastOpcode(WorldPackets::Pet::PetSpellAutocast& packet)
    {
        ...
        if (!pet)
        {
            TC_LOG_ERROR("entities.pet", "HandlePetSpellAutocastOpcode: Pet {} not found.", packet.PetGUID.ToString());
            return;
        }

        if (pet != _player->GetGuardianPet() && pet != _player->GetCharmed())
        {
            TC_LOG_ERROR("entities.pet", "HandlePetSpellAutocastOpcode: {} isn't pet of player {} ({}).",
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR("spells.pet", "HandlePetSpellAutocastOpcode: Unknown spell id {} used by {}.", packet.SpellID, packet.PetGUID.ToString());
            return;
        }
        ...
        for (Unit* petControlled : pets)
        {
            ...
            if (!charmInfo)
            {
                TC_LOG_ERROR("entities.pet", "HandlePetSpellAutocastOpcode: object {} is considered pet-like but doesn't have a charminfo!", petControlled->GetGUID().ToString());
                return;
            }
    ...
    void HandlePetCastSpellOpcode(WorldPackets::Spells::PetCastSpell& petCastSpell)
    {
        ...
        if (!caster)
        {
            TC_LOG_ERROR("entities.pet", "HandlePetCastSpellOpcode: Caster {} not found.", petCastSpell.PetGUID.ToString());
            return;
        }
        ...
        if (!spellInfo)
        {
            TC_LOG_ERROR("spells.pet", "HandlePetCastSpellOpcode: unknown spell id {} tried to cast by {}",
        ...
        {
            TC_LOG_ERROR("spells.pet", "HandlePetCastSpellOpcode: {} isn't pet of player {} ({}).", petCastSpell.PetGUID.ToString(), GetPlayer()->GetName(), GetPlayer()->GetGUID().ToString());
            return;
        }
    ...
    void SendPetNameInvalid(uint32 error, const std::string& name, Optional<DeclinedName> const& declinedName)
    {
