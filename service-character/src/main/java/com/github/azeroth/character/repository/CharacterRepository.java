...
import com.github.azeroth.character.domain.Character;
import com.github.azeroth.character.dto.CharacterAuctionItem;
...
public interface CharacterRepository extends CrudRepository<Character, Integer> {
    ...
    @Query("""
            ...
            FROM characters c LEFT JOIN character_fishingsteps cfs ON c.guid = cfs.guid WHERE c.guid =:guid
            """)
    LoginCharacter selectLoginCharacterById(@Param("guid") int guid);


    @Query("SELECT chrCustomizationOptionID, chrCustomizationChoiceID FROM character_customizations WHERE guid = :guid ORDER BY chrCustomizationOptionID")
    List<CharacterCustomization> selectCharacterCustomization(@Param("guid") int guid);

    @Query("SELECT guid FROM group_member WHERE memberGuid = :guid")
    List<Integer> selectGroupMemberGuid(@Param("guid") int guid);

    @Query("SELECT casterGuid, itemGuid, spell, effectMask, recalculateMask, difficulty, stackCount, maxDuration, remainTime, remainCharges, castItemId, castItemLevel FROM character_aura WHERE guid = :guid")
    List<CharacterAura> selectCharacterAura(@Param("guid") int guid);

    @Query("SELECT casterGuid, itemGuid, spell, effectMask, effectIndex, amount, baseAmount FROM character_aura_effect WHERE guid = :guid")
    List<CharacterAuraEffect> selectCharacterAuraEffect(@Param("guid") int guid);

    @Query("SELECT spell, active, disabled FROM character_spell WHERE guid = :guid")
    List<CharacterSpell> selectCharacterSpell(@Param("guid") int guid);

    @Query("SELECT spell FROM character_spell_favorite WHERE guid = :guid")
    List<Integer> selectCharacterSpellFavorite(@Param("guid") int guid);

    @Query("SELECT quest, status, explored, acceptTime, endTime FROM character_queststatus WHERE guid = :guid AND status <> 0")
    List<CharacterQueststatus> selectCharacterQuestStatus(@Param("guid") int guid);

    @Query("SELECT quest, objective, data FROM character_queststatus_objectives WHERE guid = :guid")
    List<CharacterQueststatusObjective> selectCharacterQuestStatusObjective(@Param("guid") int guid);

    @Query("SELECT criteriaId, counter, date FROM character_queststatus_objectives_criteria_progress WHERE guid = :guid")
    List<CharacterQueststatusObjectivesCriteriaProgress> selectCharacterQuestStatusObjectiveCriteriaProgress(@Param("guid") int guid);


    @Query("""
    SELECT ii.guid, ii.itemEntry, ii.creatorGuid, ii.giftCreatorGuid, ii.count, ii.duration, ii.charges, ii.flags, ii.enchantments, ii.randomBonusListId,
                    ...
                    im.fixedScalingLevel, im.artifactKnowledgeLevel, im.itemReforgeId, ii.owner_guid, m.id 
                    FROM mail_items mi INNER JOIN mail m ON mi.mail_id = m.id LEFT JOIN item_instance ii ON mi.item_guid = ii.guid LEFT JOIN item_instance_gems ig ON ii.guid = ig.itemGuid LEFT JOIN item_instance_transmog iit ON ii.guid = iit.itemGuid LEFT JOIN item_instance_modifiers im ON ii.guid = im.itemGuid 
                     WHERE m.receiver = :guid
     """)
    List<CharacterMailItem> selectMailItems(@Param("guid") int guid);
    ...
    @Query("""
              ...
              ii.owner_guid, ai.auctionId FROM auction_items ai INNER JOIN item_instance ii ON ai.itemGuid = ii.guid LEFT JOIN item_instance_gems ig ON ii.guid = ig.itemGuid LEFT JOIN item_instance_transmog iit ON ii.guid = iit.itemGuid LEFT JOIN item_instance_modifiers im ON ii.guid = im.itemGuid
     """)
    List<CharacterAuctionItem> selectAuctionItems(@Param("entry") int entry);
    ...
    void insertItemInstanceModifiers(@Param("itemGuid") long itemGuid,
            @Param("fixedScalingLevel") int fixedScalingLevel,
            @Param("artifactKnowledgeLevel") int artifactKnowledgeLevel, @Param("itemReforgeId") int itemReforgeId);
    ...
    void insertGuildBankItem(@Param("guildId") int guildId, @Param("tabId") int tabId, @Param("slotId") int slotId,
            @Param("itemGuid") long itemGuid, @Param("count") int count, @Param("enchanted") int enchanted,
            @Param("suffix") int suffix, @Param("propertyId") int propertyId,
            @Param("randomPropertyId") int randomPropertyId, @Param("durability") int durability);
    ...
    void insertCharacterBanned(@Param("guid") int guid, @Param("bandate") long bandate,
            @Param("unbandate") long unbandate, @Param("bannedby") String bannedby, @Param("bannedip") String bannedip,
            @Param("bannedaccount") String bannedaccount, @Param("reason") String reason, @Param("active") int active);
    ...
    void insertItemInstance(@Param("guid") long guid, @Param("itemEntry") int itemEntry,
            @Param("ownerGuid") int ownerGuid, @Param("creatorGuid") int creatorGuid,
            @Param("giftCreatorGuid") int giftCreatorGuid, @Param("count") int count, @Param("duration") int duration,
            @Param("charges") String charges, @Param("flags") int flags, @Param("enchantments") String enchantments,
            @Param("randomPropertyId") int randomPropertyId, @Param("durability") int durability,
            @Param("playedTime") int playedTime);
    ...
    void insertMail(@Param("id") int id, @Param("messageType") int messageType, @Param("stationery") int stationery,
            @Param("sender") int sender, @Param("receiver") int receiver, @Param("subject") String subject,
            @Param("body") String body, @Param("hasItems") int hasItems, @Param("expireTime") long expireTime,
            @Param("deliverTime") long deliverTime, @Param("money") long money);
    ...
    void insertCharacterPet(@Param("id") int id, @Param("entry") int entry, @Param("owner") int owner,
            @Param("modelid") int modelid, @Param("level") int level, @Param("exp") long exp,
            @Param("reactstate") int reactstate, @Param("name") String name, @Param("renametime") long renametime,
            @Param("slot") int slot, @Param("curhealth") int curhealth, @Param("curmana") int curmana,
            @Param("curhappiness") int curhappiness, @Param("loyaltypoints") int loyaltypoints,
            @Param("trainpoint") int trainpoint, @Param("spells") String spells);

    // Guild member operations
    @Query(value = "SELECT guildid, `rank` FROM guild_member WHERE guid = :guid")
    GuildMember selectGuildMember(@Param("guid") int guid);
    ...
    void insertGuildMember(@Param("guildId") int guildId, @Param("guid") int guid, @Param("rank") int rank,
            @Param("pnote") String pnote, @Param("offnote") String offnote, @Param("lastOnline") long lastOnline);
    ...
    void insertGroupMember(@Param("groupId") int groupId, @Param("memberGuid") int memberGuid,
            @Param("memberFlags") int memberFlags, @Param("assistGuid") int assistGuid);
    ...
    void insertCharacterAura(@Param("guid") int guid, @Param("spell") int spell, @Param("effectMask") int effectMask,
            @Param("stackCount") int stackCount, @Param("amount") int amount, @Param("maxDuration") int maxDuration,
            @Param("remainTime") int remainTime, @Param("appliedAt") long appliedAt, @Param("auraFlags") int auraFlags,
            @Param("casterGuid") int casterGuid);
    ...
    void insertCharacterSpell(@Param("guid") int guid, @Param("spell") int spell, @Param("active") int active,
            @Param("disabled") int disabled, @Param("cooldown") long cooldown,
            @Param("categoryCooldown") long categoryCooldown);
    ...
    void insertCharacterQuestStatus(@Param("guid") int guid, @Param("quest") int quest, @Param("status") int status,
            @Param("timer") long timer, @Param("exploration") String exploration, @Param("completed") int completed);
    ...
    void insertCharacterReputation(@Param("guid") int guid, @Param("faction") int faction,
            @Param("standing") int standing, @Param("flags") int flags);
    ...
    void insertCharacterInventory(@Param("guid") int guid, @Param("bag") int bag, @Param("slot") int slot,
            @Param("item") long item, @Param("count") int count, @Param("enchantment") int enchantment,
            @Param("durability") int durability, @Param("creator") int creator, @Param("giftCreator") int giftCreator,
            @Param("stackcount") int stackcount);
    ...
    void insertAuction(@Param("id") long id, @Param("auctionHouseId") int auctionHouseId, @Param("owner") long owner,
            @Param("bidder") long bidder, @Param("minBid") long minBid,
            @Param("buyoutOrUnitPrice") long buyoutOrUnitPrice, @Param("deposit") long deposit,
            @Param("bidAmount") long bidAmount, @Param("startTime") long startTime, @Param("endTime") long endTime,
            @Param("serverFlags") int serverFlags);

    @Modifying
    @Query("UPDATE auctionhouse SET bidder = :bidder, bidAmount = :bidAmount, serverFlags = :serverFlags WHERE id = :id")
    void updateAuctionBid(@Param("bidder") long bidder, @Param("bidAmount") long bidAmount,
            @Param("serverFlags") int serverFlags, @Param("id") long id);
    ...
    void insertAuctionhouse(@Param("id") int id, @Param("itemguid") long itemguid, @Param("owner") int owner,
            @Param("startbid") long startbid, @Param("buyoutprice") long buyoutprice,
            @Param("starttime") long starttime, @Param("endtime") long endtime, @Param("bidder") int bidder,
            @Param("bid") long bid, @Param("deposit") long deposit, @Param("auctionhouseid") int auctionhouseid,
    ...
    void insertItemInstanceGems(@Param("itemGuid") int itemGuid, @Param("gemItemId1") int gemItemId1,
            @Param("gemBonuses1") String gemBonuses1, @Param("gemContext1") int gemContext1,
            @Param("gemScalingLevel1") int gemScalingLevel1, @Param("gemItemId2") int gemItemId2,
            @Param("gemBonuses2") String gemBonuses2, @Param("gemContext2") int gemContext2,
            @Param("gemScalingLevel2") int gemScalingLevel2, @Param("gemItemId3") int gemItemId3,
            @Param("gemBonuses3") String gemBonuses3, @Param("gemContext3") int gemContext3,
            @Param("gemScalingLevel3") int gemScalingLevel3);
    ...
    void insertItemInstanceTransmog(@Param("itemGuid") int itemGuid,
            @Param("itemModifiedAppearanceAllSpecs") int itemModifiedAppearanceAllSpecs,
            @Param("itemModifiedAppearanceSpec1") int itemModifiedAppearanceSpec1,
            @Param("itemModifiedAppearanceSpec2") int itemModifiedAppearanceSpec2,
            @Param("itemModifiedAppearanceSpec3") int itemModifiedAppearanceSpec3,
            @Param("itemModifiedAppearanceSpec4") int itemModifiedAppearanceSpec4,
            @Param("itemModifiedAppearanceSpec5") int itemModifiedAppearanceSpec5);
    ...
    void insertGuildBankTab(@Param("guildId") int guildId, @Param("tabId") int tabId, @Param("name") String name,
            @Param("icon") String icon, @Param("tabPermissions") String tabPermissions, @Param("viewed") int viewed);
    ...
    void insertCharacterEquipmentSet(@Param("guid") int guid, @Param("setguid") int setguid,
            @Param("setname") String setname, @Param("seticon") int seticon, @Param("items") String items);
    ...
    CharacterBattlegroundDatum selectCharacterBattlegroundData(@Param("guid") int guid);
    ...
    void insertCharacterBattlegroundData(@Param("guid") long guid, @Param("instanceId") int instanceId,
            @Param("team") int team, @Param("joinX") float joinX, @Param("joinY") float joinY,
            @Param("joinZ") float joinZ, @Param("joinO") float joinO, @Param("joinMapId") int joinMapId,
            @Param("taxiStart") int taxiStart, @Param("taxiEnd") int taxiEnd, @Param("mountSpell") int mountSpell,
    ...
    List<ArenaTeamMember> selectCharacterArenaInfo(@Param("guid") int guid);
    ...
    void insertArenaTeam(@Param("arenaTeamId") Integer arenaTeamId, @Param("name") String name,
            @Param("captainGuid") Long captainGuid, @Param("type") Integer type, @Param("rating") Integer rating,
            @Param("backgroundColor") Integer backgroundColor, @Param("emblemStyle") Integer emblemStyle,
            @Param("emblemColor") Integer emblemColor, @Param("borderStyle") Integer borderStyle,
            @Param("borderColor") Integer borderColor);

    @Modifying
    @Query(value = "INSERT INTO arena_team_member (arenaTeamId, guid, personalRating) VALUES (:arenaTeamId, :guid, :personalRating)")
    void insertArenaTeamMember(@Param("arenaTeamId") Integer arenaTeamId, @Param("guid") Long guid,
            @Param("personalRating") Integer personalRating);
    ...
    void updateArenaTeamStats(@Param("arenaTeamId") Integer arenaTeamId, @Param("rating") Integer rating,
            @Param("weekGames") Integer weekGames, @Param("weekWins") Integer weekWins,
            @Param("seasonGames") Integer seasonGames, @Param("seasonWins") Integer seasonWins,
    ...
    void updateArenaTeamMember(@Param("arenaTeamId") Integer arenaTeamId, @Param("guid") Long guid,
            @Param("personalRating") Integer personalRating, @Param("weekGames") Integer weekGames,
            @Param("weekWins") Integer weekWins, @Param("seasonGames") Integer seasonGames,
            @Param("seasonWins") Integer seasonWins);
    ...
    void replaceCharacterArenaStats(@Param("guid") Long guid, @Param("slot") Integer slot,
            @Param("matchMakerRating") Integer matchMakerRating);
    ...
    void insertArenaTeam(@Param("arenaTeamId") int arenaTeamId, @Param("name") String name,
            @Param("captain") int captain, @Param("type") int type, @Param("emblemStyle") int emblemStyle,
            @Param("emblemColor") int emblemColor, @Param("borderStyle") int borderStyle,
            @Param("borderColor") int borderColor, @Param("backgroundColor") int backgroundColor);
    ...
    void insertCharacterTransmogOutfit(@Param("guid") int guid, @Param("outfitId") int outfitId,
            @Param("name") String name, @Param("items") String items);

    // Character currency operations
    @Query("SELECT Currency, Quantity, WeeklyQuantity, TrackedQuantity, IncreasedCapQuantity, EarnedQuantity, Flags FROM character_currency WHERE CharacterGuid = :guid")
    List<CharacterCurrency> selectCharacterCurrency(@Param("guid") int guid);

    @Modifying
    @Query("UPDATE character_currency SET Quantity = :quantity, WeeklyQuantity = :weeklyQuantity, TrackedQuantity = :trackedQuantity, IncreasedCapQuantity = :increasedCapQuantity, EarnedQuantity = :earnedQuantity, Flags = :flags WHERE CharacterGuid = :guid AND Currency = :currency")
    void updateCharacterCurrency(@Param("guid") int guid, @Param("currency") int currency, @Param("quantity") int quantity,
            @Param("weeklyQuantity") int weeklyQuantity, @Param("trackedQuantity") int trackedQuantity,
            @Param("increasedCapQuantity") int increasedCapQuantity, @Param("earnedQuantity") int earnedQuantity,
            @Param("flags") int flags);

    @Modifying
    @Query("REPLACE INTO character_currency (CharacterGuid, Currency, Quantity, WeeklyQuantity, TrackedQuantity, IncreasedCapQuantity, EarnedQuantity, Flags) VALUES (:guid, :currency, :quantity, :weeklyQuantity, :trackedQuantity, :increasedCapQuantity, :earnedQuantity, :flags)")
    void replaceCharacterCurrency(@Param("guid") int guid, @Param("currency") int currency, @Param("quantity") int quantity,
            @Param("weeklyQuantity") int weeklyQuantity, @Param("trackedQuantity") int trackedQuantity,
            @Param("increasedCapQuantity") int increasedCapQuantity, @Param("earnedQuantity") int earnedQuantity,
            @Param("flags") int flags);

    @Modifying
    @Query("DELETE FROM character_currency WHERE CharacterGuid = :guid")
    void deleteCharacterCurrency(@Param("guid") int guid);
    ...
    void insertCharacterCurrency(@Param("guid") int guid, @Param("currency") int currency, @Param("count") int count,
            @Param("weeklyCount") int weeklyCount, @Param("seasonCount") int seasonCount, @Param("flags") int flags);

    @Query("SELECT type, time, data FROM account_data WHERE accountId = :accountId")
    List<AccountData> selectAccountData(@Param("accountId") int accountId);
    ...
    void insertAccountData(@Param("accountId") int accountId, @Param("type") int type, @Param("time") long time,
    ...
    void insertCharacterAccountData(@Param("guid") int guid, @Param("type") int type, @Param("time") long time,
            @Param("data") String data);

    @Query("SELECT type, time, data FROM character_account_data WHERE guid = :guid")
    List<CharacterAccountDatum> selectCharacterAccountData(@Param("guid") int guid);
    ...
    void insertGameEventSave(@Param("eventEntry") int eventEntry, @Param("state") int state,
            @Param("nextStart") long nextStart);
    ...
    void insertPetition(@Param("petitionId") int petitionId, @Param("name") String name,
            @Param("ownerGuid") int ownerGuid, @Param("type") int type, @Param("allowedGuildID") int allowedGuildID,
            @Param("signCount") int signCount, @Param("maxSignCount") int maxSignCount,
            @Param("deadline") long deadline, @Param("issueDate") long issueDate, @Param("isClosed") int isClosed);
    ...
    void insertArenaTeamMember(@Param("arenaTeamId") int arenaTeamId, @Param("guid") int guid,
            @Param("weekGames") int weekGames, @Param("weekWins") int weekWins, @Param("seasonGames") int seasonGames,
            @Param("seasonWins") int seasonWins, @Param("personalRating") int personalRating);

    // Petition sign operations
    @Query(value = "SELECT achievement, date FROM character_achievement WHERE guid = :guid")
    List<CharacterAchievement> selectCharacterAchievements(@Param("guid") int guid);

    @Query(value = "SELECT criteria, counter, date FROM character_achievement_progress WHERE guid = :guid")
    List<CharacterAchievementProgress> selectCharacterCriteriaProgress(@Param("guid") int guid);

    @Query(value = "SELECT setguid, setindex, name, iconname, ignore_mask, AssignedSpecIndex, item0, item1, item2, item3, item4, item5, item6, item7, item8, " +
            "item9, item10, item11, item12, item13, item14, item15, item16, item17, item18 FROM character_equipmentsets WHERE guid = :guid ORDER BY setindex")
    List<CharacterEquipmentSet> selectCharacterEquipmentSets(@Param("guid") int guid);
    ...
    @Query(value = """
            ...
            appearance17, appearance18 FROM character_transmog_outfits WHERE guid = :guid ORDER BY setindex
            """)
    List<CharacterTransmogOutfit> selectCharacterTransmogOutfits(@Param("guid") int guid);


    @Query(value = "SELECT talentGroup, glyphSlot, glyphId FROM character_glyphs WHERE guid = :guid")
    List<CharacterGlyph> selectCharacterGlyphs(@Param("guid") int guid);

    @Query(value = "SELECT talentGroup, talentId, `rank` FROM character_talent WHERE guid = :guid")
    List<CharacterTalent> selectCharacterTalents(@Param("guid") int guid);

    @Query(value = "SELECT id, talentTabId FROM character_talent_group WHERE guid = :guid ORDER BY id ASC")
    List<CharacterTalentGroup> selectCharacterTalentGroups(@Param("guid") int guid);
    ...
    void insertPetitionSign(@Param("petitionId") int petitionId, @Param("playerGuid") int playerGuid,
    ...
    void insertCharacterSkill(@Param("guid") int guid, @Param("skill") int skill, @Param("value") int value,
            @Param("maxValue") int maxValue, @Param("step") int step, @Param("rank") int rank,
    ...
    List<CharacterSocial> selectCharacterSocialList(@Param("guid") int guid);
    ...
    void insertCharacterSocial(@Param("guid") int guid, @Param("friendGuid") int friendGuid, @Param("flags") int flags,
    ...
    void insertGMComplaint(@Param("complaintId") int complaintId, @Param("guid") int guid,
            @Param("reportedGuid") int reportedGuid, @Param("type") int type, @Param("comment") String comment,
            @Param("createTime") long createTime, @Param("mapId") int mapId, @Param("posX") float posX,
            @Param("posY") float posY, @Param("posZ") float posZ);
    ...
    void replaceRespawn(@Param("type") int type, @Param("spawnId") int spawnId, @Param("respawnTime") Long respawnTime,
            @Param("mapId") int mapId, @Param("instanceId") Integer instanceId);

    @Modifying
    @Query(value = "DELETE FROM respawn WHERE type = :type AND spawnId = :spawnId AND mapId = :mapId AND instanceId = :instanceId")
    @Transactional
    void deleteRespawn(@Param("type") int type, @Param("spawnId") int spawnId, @Param("mapId") int mapId,
    ...
    void replaceGMBug(@Param("id") Integer id, @Param("playerGuid") Long playerGuid, @Param("note") String note,
            @Param("mapId") int mapId, @Param("posX") Float posX, @Param("posY") Float posY, @Param("posZ") Float posZ,
            @Param("facing") Float facing, @Param("closedBy") Long closedBy, @Param("assignedTo") Long assignedTo,
    ...
    void replaceGMComplaint(@Param("id") Integer id, @Param("playerGuid") Long playerGuid, @Param("note") String note,
            @Param("mapId") int mapId, @Param("posX") Float posX, @Param("posY") Float posY, @Param("posZ") Float posZ,
            @Param("facing") Float facing, @Param("targetCharacterGuid") Long targetCharacterGuid,
            @Param("reportType") Integer reportType, @Param("reportMajorCategory") Integer reportMajorCategory,
            @Param("reportMinorCategoryFlags") Integer reportMinorCategoryFlags,
            @Param("reportLineIndex") Integer reportLineIndex, @Param("assignedTo") Long assignedTo,
            @Param("closedBy") Long closedBy, @Param("comment") String comment);
    ...
    void insertGMComplaintChatline(@Param("complaintId") Integer complaintId, @Param("lineId") Integer lineId,
            @Param("timestamp") Long timestamp, @Param("text") String text);
    ...
    void replaceGMSuggestion(@Param("id") Integer id, @Param("playerGuid") Long playerGuid, @Param("note") String note,
            @Param("mapId") int mapId, @Param("posX") Float posX, @Param("posY") Float posY, @Param("posZ") Float posZ,
            @Param("facing") Float facing, @Param("closedBy") Long closedBy, @Param("assignedTo") Long assignedTo,
            @Param("comment") String comment);
    ...
    void insertGMBug(@Param("bugId") int bugId, @Param("guid") int guid, @Param("type") int type,
            @Param("content") String content, @Param("createTime") long createTime, @Param("mapId") int mapId,
            @Param("posX") float posX, @Param("posY") float posY, @Param("posZ") float posZ);

    // Tutorials
    @Query(value = "SELECT tut0, tut1, tut2, tut3, tut4, tut5, tut6, tut7 FROM account_tutorial WHERE accountId = :accountId")
    AccountTutorial selectAccountTutorial(@Param("accountId") Integer accountId);
    ...
    void updateTutorials(AccountTutorial accountTutorial);
    ...
    void insertGameEventSave(@Param("eventEntry") Integer eventEntry, @Param("state") Integer state,
            @Param("next_start") Instant nextStart);
    ...
    void deleteGameEventConditionSave(@Param("eventEntry") Integer eventEntry,
    ...
    void insertGameEventConditionSave(@Param("eventEntry") Integer eventEntry,
            @Param("conditionId") Integer conditionId, @Param("done") Integer done);
    ...
    List<Long> selectPetitionSigByAccount(@Param("playerAccount") Long playerAccount,
            @Param("petitionguid") Long petitionguid);
    ...
    void insertPetitionSignature(@Param("ownerGuid") long ownerGuid, @Param("petitionGuid") long petitionGuid,
            @Param("playerGuid") long playerGuid, @Param("playerAccount") long playerAccount);
    ...
    void insertCharacterCustomization(@Param("guid") long guid, @Param("optionId") int optionId,
    ...
    void insertGroup(@Param("guid") long guid, @Param("leaderGuid") long leaderGuid,
            @Param("lootMethod") int lootMethod, @Param("looterGuid") long looterGuid,
            @Param("lootThreshold") int lootThreshold, @Param("icon1") int icon1, @Param("icon2") int icon2,
            @Param("icon3") int icon3, @Param("icon4") int icon4, @Param("icon5") int icon5, @Param("icon6") int icon6,
            @Param("icon7") int icon7, @Param("icon8") int icon8, @Param("groupType") int groupType,
            @Param("difficulty") int difficulty, @Param("raidDifficulty") int raidDifficulty,
            @Param("legacyRaidsDifficulty") int legacyRaidsDifficulty,
    ...
    void insertGroupMember(@Param("guid") long guid, @Param("memberGuid") long memberGuid,
            @Param("memberFlags") int memberFlags, @Param("subgroup") int subgroup, @Param("roles") int roles);
    ...
    void updateGroupLegacyRaidDifficulty(@Param("legacyRaidsDifficulty") int legacyRaidsDifficulty,
    ...
    void updateCharacterNameAtLogin(@Param("name") String name, @Param("atLogin") int atLogin,
            @Param("guid") long guid);
    ...
    CharacterHomebind selectCharacterHomebind(@Param("guid") long guid);
    ...
    List<Mail> selectMail(@Param("receiver") long receiver);
    ...
    List<Map<String, Object>> selectCharacterInventoryItemByEntry(@Param("itemEntry") int itemEntry,
    ...
    List<Map<String, Object>> selectMailItems(@Param("itemEntry") int itemEntry, @Param("limit") int limit);
    ...
    List<Map<String, Object>> selectAuctionhouseItemByEntry(@Param("itemEntry") int itemEntry,
            @Param("limit") int limit);
    ...
    void insertCharacterAchievement(@Param("guid") long guid, @Param("achievement") int achievement,
    ...
    void insertCharacterAchievementProgress(@Param("guid") long guid, @Param("criteria") int criteria,
            @Param("counter") int counter, @Param("date") long date);
    ...
    void insertCharacterReputationByFaction(@Param("guid") long guid, @Param("faction") int faction,
            @Param("standing") int standing, @Param("flags") int flags);
    ...
    void insertItemRefundInstance(@Param("itemGuid") long itemGuid, @Param("playerGuid") long playerGuid,
            @Param("paidMoney") long paidMoney, @Param("paidExtendedCost") int paidExtendedCost);
    ...
    void insertCharacterGift(@Param("guid") long guid, @Param("itemGuid") long itemGuid, @Param("entry") int entry,
            @Param("flags") int flags);
    ...
    void insertPetition(@Param("ownerGuid") long ownerGuid, @Param("petitionGuid") long petitionGuid,
    ...
    CharacterDeclinedname selectCharacterDeclinedname(@Param("guid") long guid);
    ...
    void insertCharacterDeclinedName(@Param("guid") long guid, @Param("genitive") String genitive,
            @Param("dative") String dative, @Param("accusative") String accusative,
            @Param("instrumental") String instrumental, @Param("prepositional") String prepositional);
    ...
    void insertCharacterQueststatusSeasonal(@Param("guid") long guid, @Param("quest") int quest,
            @Param("event") int event, @Param("completedTime") long completedTime);
    ...
    void deleteCharacterQueststatusSeasonalByEventAndTime(@Param("event") int event,
    ...
    void deleteCharacterQueststatusObjectivesCriteriaProgressByGuidAndCriteriaId(@Param("guid") long guid,
            @Param("criteriaId") int criteriaId);

    @Query(value = "SELECT criteriaId, counter, date FROM character_queststatus_objectives_criteria_progress WHERE guid = :guid")
    List<CharacterQueststatusObjectivesCriteriaProgress> selectCharacterQueststatusObjectivesCriteriaProgress(
            @Param("guid") long guid);
    ...
    void updateCharacterAchievement(@Param("newAchievement") int newAchievement,
            @Param("oldAchievement") int oldAchievement, @Param("guid") long guid);
    ...
    void updateCharacterInventoryFactionChange(@Param("newItemEntry") int newItemEntry,
            @Param("oldItemEntry") int oldItemEntry, @Param("guid") long guid);
    ...
    void updateCharacterSpellFactionChange(@Param("newSpell") int newSpell, @Param("oldSpell") int oldSpell,
            @Param("guid") long guid);
    ...
    void updateCharacterRepFactionChange(@Param("newFaction") int newFaction, @Param("standing") int standing,
            @Param("oldFaction") int oldFaction, @Param("guid") long guid);
    ...
    void insertCharacterSpellCooldown(@Param("guid") long guid, @Param("spell") int spell, @Param("item") long item,
            @Param("time") long time, @Param("categoryId") int categoryId, @Param("categoryEnd") long categoryEnd);
    ...
    void insertCharacterSpellCharges(@Param("guid") long guid, @Param("categoryId") int categoryId,
            @Param("rechargeStart") long rechargeStart, @Param("rechargeEnd") long rechargeEnd);
    ...
    void insertCharacterAction(@Param("guid") long guid, @Param("spec") int spec,
            @Param("traitConfigId") int traitConfigId, @Param("button") int button, @Param("action") int action,
            @Param("type") int type);
    ...
    void updateCharacterAction(@Param("action") int action, @Param("type") int type, @Param("guid") long guid,
            @Param("button") int button, @Param("spec") int spec, @Param("traitConfigId") int traitConfigId);

    @Modifying
    @Query(value = "DELETE FROM character_action WHERE guid = :guid AND button = :button AND spec = :spec AND traitConfigId = :traitConfigId")
    void deleteCharacterActionByButtonSpec(@Param("guid") long guid, @Param("button") int button,
            @Param("spec") int spec, @Param("traitConfigId") int traitConfigId);
    ...
    void updateMail(@Param("hasItems") int hasItems, @Param("expireTime") long expireTime,
            @Param("deliverTime") long deliverTime, @Param("money") long money, @Param("cod") long cod,
            @Param("checked") int checked, @Param("id") long id);
    ...
    void replaceCharacterQueststatus(@Param("guid") long guid, @Param("quest") int quest, @Param("status") int status,
            @Param("explored") int explored, @Param("acceptTime") long acceptTime, @Param("endTime") long endTime);
    ...
    void replaceCharacterQueststatusObjectives(@Param("guid") long guid, @Param("quest") int quest,
            @Param("objective") int objective, @Param("data") int data);
    ...
    void insertCharacterQueststatusObjectivesCriteria(@Param("guid") long guid,
            @Param("questObjectiveId") int questObjectiveId);
    ...
    void insertCharacterQueststatusObjectivesCriteriaProgress(@Param("guid") long guid,
            @Param("criteriaId") int criteriaId, @Param("counter") int counter, @Param("date") long date);
    ...
    void updateCharacterQueststatusRewardedFactionChange(@Param("newQuest") int newQuest,
            @Param("oldQuest") int oldQuest, @Param("guid") long guid);
    ...
    void insertCharacterSkills(@Param("guid") long guid, @Param("skill") int skill, @Param("value") int value,
            @Param("max") int max, @Param("professionSlot") int professionSlot);
    ...
    void updateCharacterSkills(@Param("value") int value, @Param("max") int max,
            @Param("professionSlot") int professionSlot, @Param("guid") long guid, @Param("skill") int skill);

    @Modifying
    @Query(value = "INSERT INTO character_spell (guid, spell, active, disabled) VALUES (:guid, :spell, :active, :disabled)")
    void insertCharacterSpell(@Param("guid") long guid, @Param("spell") int spell, @Param("active") int active,
    ...
    void insertCharacterStats(@Param("guid") long guid, @Param("maxhealth") int maxhealth,
            @Param("maxpower1") int maxpower1, @Param("maxpower2") int maxpower2, @Param("maxpower3") int maxpower3,
            @Param("maxpower4") int maxpower4, @Param("maxpower5") int maxpower5, @Param("maxpower6") int maxpower6,
            @Param("maxpower7") int maxpower7, @Param("maxpower8") int maxpower8, @Param("maxpower9") int maxpower9,
            @Param("maxpower10") int maxpower10, @Param("strength") int strength, @Param("agility") int agility,
            @Param("stamina") int stamina, @Param("intellect") int intellect, @Param("armor") int armor,
            @Param("resHoly") int resHoly, @Param("resFire") int resFire, @Param("resNature") int resNature,
            @Param("resFrost") int resFrost, @Param("resShadow") int resShadow, @Param("resArcane") int resArcane,
            @Param("blockPct") float blockPct, @Param("dodgePct") float dodgePct, @Param("parryPct") float parryPct,
            @Param("critPct") float critPct, @Param("rangedCritPct") float rangedCritPct,
            @Param("spellCritPct") float spellCritPct, @Param("attackPower") int attackPower,
            @Param("rangedAttackPower") int rangedAttackPower, @Param("spellPower") int spellPower,
            @Param("resilience") int resilience, @Param("mastery") float mastery,
            @Param("versatility") float versatility);
    ...
    void insertCharacterGlyphs(@Param("guid") long guid, @Param("talentGroup") int talentGroup,
            @Param("glyphSlot") int glyphSlot, @Param("glyphId") int glyphId);

    @Modifying
    @Query(value = "INSERT INTO character_talent (guid, talentGroup, talentId, `rank`) VALUES (:guid, :talentGroup, :talentId, :rank)")
    void insertCharacterTalent(@Param("guid") long guid, @Param("talentGroup") int talentGroup,
            @Param("talentId") int talentId, @Param("rank") int rank);

    @Modifying
    @Query(value = "INSERT INTO character_talent_group (guid, id, talentTabId) VALUES (:guid, :id, :talentTabId)")
    void insertCharacterTalentGroup(@Param("guid") long guid, @Param("id") int id,
            @Param("talentTabId") int talentTabId);
    ...
    void replaceCharacterVoidStorageItem(@Param("itemId") long itemId, @Param("playerGuid") long playerGuid,
            @Param("itemEntry") int itemEntry, @Param("slot") int slot, @Param("creatorGuid") long creatorGuid,
            @Param("randomBonusListId") int randomBonusListId, @Param("fixedScalingLevel") int fixedScalingLevel,
            @Param("artifactKnowledgeLevel") int artifactKnowledgeLevel, @Param("context") int context,
            @Param("bonusListIDs") String bonusListIDs);
    ...
    List<CharacterCufProfile> selectCharacterCufProfiles(@Param("guid") int guid);
    ...
    void replaceCharacterCufProfiles(@Param("guid") long guid, @Param("id") int id, @Param("name") String name,
            @Param("frameHeight") int frameHeight, @Param("frameWidth") int frameWidth, @Param("sortBy") int sortBy,
            @Param("healthText") int healthText, @Param("boolOptions") int boolOptions, @Param("topPoint") int topPoint,
            @Param("bottomPoint") int bottomPoint, @Param("leftPoint") int leftPoint, @Param("topOffset") int topOffset,
            @Param("bottomOffset") int bottomOffset, @Param("leftOffset") int leftOffset);
    ...
    void deleteItemcontainerItem(@Param("containerId") long containerId, @Param("itemType") int itemType,
            @Param("itemId") long itemId, @Param("itemCount") int itemCount, @Param("itemIndex") int itemIndex);
    ...
    void insertItemcontainerItems(@Param("containerId") long containerId, @Param("itemType") int itemType,
            @Param("itemId") long itemId, @Param("itemCount") int itemCount, @Param("itemIndex") int itemIndex,
            @Param("followRules") int followRules, @Param("ffa") int ffa, @Param("blocked") int blocked,
            @Param("counted") int counted, @Param("underThreshold") int underThreshold,
            @Param("needsQuest") int needsQuest, @Param("rndBonus") int rndBonus, @Param("context") int context,
            @Param("bonusListIds") String bonusListIds);
    ...
    void replaceCalendarEvent(@Param("eventId") long eventId, @Param("owner") long owner, @Param("title") String title,
            @Param("description") String description, @Param("eventType") int eventType,
            @Param("textureId") int textureId, @Param("date") long date, @Param("flags") int flags,
            @Param("lockDate") long lockDate);
    ...
    void replaceCalendarInvite(@Param("inviteId") long inviteId, @Param("eventId") long eventId,
            @Param("invitee") long invitee, @Param("sender") long sender, @Param("status") int status,
            @Param("responseTime") long responseTime, @Param("moderationRank") int moderationRank,
            @Param("note") String note);
    ...
    void insertCharacterPetDeclinedname(@Param("id") long id, @Param("owner") long owner,
            @Param("genitive") String genitive, @Param("dative") String dative, @Param("accusative") String accusative,
            @Param("instrumental") String instrumental, @Param("prepositional") String prepositional);
    ...
    void insertPetSpellCooldown(@Param("guid") long guid, @Param("spell") int spell, @Param("time") long time,
            @Param("categoryId") int categoryId, @Param("categoryEnd") long categoryEnd);
    ...
    void insertPetSpellCharges(@Param("guid") long guid, @Param("categoryId") int categoryId,
            @Param("rechargeStart") long rechargeStart, @Param("rechargeEnd") long rechargeEnd);
    ...
    void insertPetAura(@Param("guid") long guid, @Param("casterGuid") long casterGuid, @Param("spell") int spell,
            @Param("effectMask") int effectMask, @Param("recalculateMask") int recalculateMask,
            @Param("difficulty") int difficulty, @Param("stackCount") int stackCount,
            @Param("maxDuration") int maxDuration, @Param("remainTime") int remainTime,
            @Param("remainCharges") int remainCharges);
    ...
    void insertPetAuraEffect(@Param("guid") long guid, @Param("casterGuid") long casterGuid, @Param("spell") int spell,
            @Param("effectMask") int effectMask, @Param("effectIndex") int effectIndex, @Param("amount") int amount,
            @Param("baseAmount") int baseAmount);
    ...
    List<CharacterPet> selectCharacterPets(@Param("owner") long owner);
    ...
    void insertPet(@Param("id") long id, @Param("entry") int entry, @Param("owner") long owner,
            @Param("modelid") int modelid, @Param("level") int level, @Param("exp") long exp,
            @Param("Reactstate") int Reactstate, @Param("slot") int slot, @Param("name") String name,
            @Param("renamed") int renamed, @Param("curhealth") int curhealth, @Param("curmana") int curmana,
            @Param("abdata") String abdata, @Param("savetime") long savetime,
            @Param("CreatedBySpell") int CreatedBySpell, @Param("PetType") int PetType,
            @Param("specialization") int specialization);
    ...
    void insertPvpstatsBattleground(@Param("id") long id, @Param("winnerFaction") int winnerFaction,
            @Param("bracketId") int bracketId, @Param("type") int type);
    ...
    void insertPvpstatsPlayer(@Param("battlegroundId") long battlegroundId, @Param("characterGuid") long characterGuid,
            @Param("winner") int winner, @Param("scoreKillingBlows") int scoreKillingBlows,
            @Param("scoreDeaths") int scoreDeaths, @Param("scoreHonorableKills") int scoreHonorableKills,
            @Param("scoreBonusHonor") int scoreBonusHonor, @Param("scoreDamageDone") int scoreDamageDone,
            @Param("scoreHealingDone") int scoreHealingDone, @Param("attr1") int attr1, @Param("attr2") int attr2,
            @Param("attr3") int attr3, @Param("attr4") int attr4, @Param("attr5") int attr5);
    ...
    void insertQuestTrack(@Param("id") int id, @Param("characterGuid") long characterGuid,
            @Param("coreHash") String coreHash, @Param("coreRevision") String coreRevision);
    ...
    List<CharacterSpellCooldown> selectCharacterSpellCooldowns(@Param("guid") int guid);
    ...
    List<CharacterSpellCharge> selectCharacterSpellCharges(@Param("guid") int guid);

    @Query(value = "SELECT skill, value, max, professionSlot FROM character_skills WHERE guid = :guid")
    List<CharacterSkill> selectCharacterSkills(@Param("guid") int guid);

    @Query(value = "SELECT guid FROM character_battleground_random WHERE guid = :guid")
    List<CharacterBattlegroundRandom> selectCharacterBattlegroundRandom(@Param("guid") int guid);

    @Query(value = "SELECT guid FROM character_banned WHERE guid = :guid AND active = 1")
    List<CharacterBanned> selectCharacterBanned(@Param("guid") int guid);
    ...
    // Corpse location operations
    @Query(value = "SELECT posX, posY, posZ, orientation, mapId, displayId, itemCache, race, class, gender, flags, dynFlags, time, corpseType, instanceId, guid FROM corpse WHERE mapId = :mapId AND instanceId = :instanceId")
    List<Corpse> selectCorpseLocation(@Param("mapId") int mapId, @Param("instanceId") int instanceId);

    @Query(value = "SELECT mapId, posX, posY, posZ, orientation FROM corpse WHERE guid = :guid")
    List<Corpse> selectCorpseLocation(@Param("guid") int guid);
    ...
    @Query(value = "SELECT instanceId, releaseTime FROM account_instance_times WHERE accountId = :accountId")
    List<AccountInstanceTime> selectAccountInstance(@Param("accountId") int accountId);

    @Query(value = "REPLACE INTO character_inventory (guid, bag, slot, item) VALUES (:guid, :bag, :slot, :item)")
    void replaceCharacterInventory(@Param("guid") int guid, @Param("bag") int bag, @Param("slot") int slot,
            @Param("item") int item);
    ...
    void replaceItemInstance(@Param("itemEntry") int itemEntry, @Param("ownerGuid") int ownerGuid,
            @Param("creatorGuid") int creatorGuid, @Param("giftCreatorGuid") int giftCreatorGuid,
            @Param("count") int count, @Param("duration") int duration, @Param("charges") String charges,
            @Param("flags") int flags, @Param("enchantments") String enchantments,
            @Param("randomBonusListId") int randomBonusListId, @Param("durability") int durability,
            @Param("playedTime") int playedTime, @Param("createTime") int createTime, @Param("text") String text,
            @Param("battlePetSpeciesId") int battlePetSpeciesId, @Param("battlePetBreedData") int battlePetBreedData,
            @Param("battlePetLevel") int battlePetLevel, @Param("battlePetDisplayId") int battlePetDisplayId,
            @Param("context") int context, @Param("bonusListIDs") String bonusListIDs,
            @Param("randomPropertiesId") int randomPropertiesId, @Param("guid") int guid);
    ...
    void updateItemInstance(@Param("itemEntry") int itemEntry, @Param("ownerGuid") int ownerGuid,
            @Param("creatorGuid") int creatorGuid, @Param("giftCreatorGuid") int giftCreatorGuid,
            @Param("count") int count, @Param("duration") int duration, @Param("charges") String charges,
            @Param("flags") int flags, @Param("enchantments") String enchantments,
            @Param("randomBonusListId") int randomBonusListId, @Param("durability") int durability,
    ...
    void updateBlackmarketAuctions(@Param("currentBid") long currentBid, @Param("time") long time,
            @Param("numBids") int numBids, @Param("bidder") long bidder, @Param("marketId") long marketId);
    ...
    void insertBlackmarketAuctions(@Param("marketId") long marketId, @Param("currentBid") long currentBid,
            @Param("time") long time, @Param("numBids") int numBids, @Param("bidder") long bidder);
    ...
    List<CharacterAuraStoredLocation> selectCharacterAuraStoredLocations(@Param("guid") long guid);
    ...
    void insertCharacterAuraStoredLocation(@Param("guid") long guid, @Param("spell") int spell,
            @Param("mapId") int mapId, @Param("positionX") float positionX, @Param("positionY") float positionY,
            @Param("positionZ") float positionZ, @Param("orientation") float orientation);
    ...
    void insertCharacterInstanceLock(@Param("guid") long guid, @Param("mapId") int mapId, @Param("lockId") int lockId,
            @Param("instanceId") int instanceId, @Param("difficulty") int difficulty, @Param("data") String data,
            @Param("completedEncountersMask") long completedEncountersMask,
            @Param("entranceWorldSafeLocId") int entranceWorldSafeLocId, @Param("expiryTime") long expiryTime,
            @Param("extended") int extended);

    @Modifying
    @Query(value = "UPDATE character_instance_lock SET extended = :extended WHERE guid = :guid AND mapId = :mapId AND lockId = :lockId")
    void updateCharacterInstanceLockExtension(@Param("extended") int extended, @Param("guid") long guid,
            @Param("mapId") int mapId, @Param("lockId") int lockId);
    ...
    void updateCharacterInstanceLockForceExpire(@Param("expiryTime") long expiryTime, @Param("guid") long guid,
            @Param("mapId") int mapId, @Param("lockId") int lockId);
    ...
    void insertInstance(@Param("instanceId") int instanceId, @Param("data") String data,
            @Param("completedEncountersMask") long completedEncountersMask,
            @Param("entranceWorldSafeLocId") int entranceWorldSafeLocId);
