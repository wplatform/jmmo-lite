...
public final class BNetAccountManager implements BNetAccountService {

    private final BNetAccountRepository bNetAccountRepo;

    private final AccountManager accountService;

    public AccountOpResult createBNetAccount(String email, String password, boolean withGameAccount, YieldResult<String> gameAccountName) {
        ...
        bNetAccount.setSrpVersion((byte)2);
        bNetAccount.setVerifier();
        bNetAccount.setSalt();
        bNetAccount.setEmail(email.toUpperCase());


        bNetAccountRepo.save(bNetAccount);



        if (withGameAccount) {
            gameAccountName.set(bNetAccount.getId() + "#1");
            accountService.createAccount(gameAccountName.get(),password, email, bNetAccount.getId(), (byte) 1 );
        }
