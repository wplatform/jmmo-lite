...
public final class PlayerConditions {
    ...
    public boolean isPlayerMeetingCondition(Player player, int conditionId) {
        if (conditionId == 0)
            return true;
    ...
    public boolean isPlayerMeetingCondition(Player player, PlayerCondition condition) {
        ...
        if (condition.getWeaponSubclassMask() != 0) {
            var mainHand = player.getItemByPos(PlayerDefine.INVENTORY_SLOT_BAG_0, PlayerDefine.EQUIPMENT_SLOT_MAINHAND);

            if (mainHand == null || ((1 << mainHand.getTemplate().getSubClass()) & condition.getWeaponSubclassMask()) == 0)
                return false;
        }



        if (condition.getPartyStatus() != 0) {
            var group = player.getGroup();
            switch (condition.getPartyStatus()) {
                case 1:
                    if (group != null)
                        return false;
                    break;
        ...
        if (condition.getPrevQuestID()[0] != 0) {
            boolean[] results = new boolean[condition.getPrevQuestID().length];
            Arrays.fill(results, true);
            for (int i = 0; i < condition.getPrevQuestID().length; ++i)
            results[i] = player.isQuestCompletedBitSet(condition.getPrevQuestID()[i]);

            if (!playerConditionLogic(condition.getPrevQuestLogic(), results))
                return false;
        }
    ...
    private static int worldStateExpressionFunction(WorldStateExpressionFunction functionType, Player player, int arg1, int arg2) {
        switch (functionType) {
            ...
            case OLD_DIFFICULTY_ID:
                ...
                return -1;
            case HOLIDAY_ACTIVE:
                return global.getGameEventMgr().isHolidayActive(HolidayId.forValue(arg1)) ? 1 : 0;
