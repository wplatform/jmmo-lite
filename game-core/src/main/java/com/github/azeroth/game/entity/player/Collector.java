...
import com.github.azeroth.auth.domain.BattlenetAccountHeirloom;
import com.github.azeroth.auth.domain.BattlenetAccountMount;
import com.github.azeroth.auth.domain.BattlenetAccountToy;
import com.github.azeroth.auth.domain.BattlenetItemAppearance;
...
import com.github.azeroth.dbc.domain.Heirloom;
...
import com.github.azeroth.game.networking.packet.misc.AccountMountUpdate;
...
public class Collector {
    ...
    private final int HEIRLOOM_FLAG_NONE                    = 0x00;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_1         = 0x01;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_2         = 0x02;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_3         = 0x04;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_4         = 0x08;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_5         = 0x10;
    private final int HEIRLOOM_FLAG_UPGRADE_LEVEL_6         = 0x20;

    private final int HEIRLOOM_ITEM_FLAG_NONE               = 0x00;
    private final int HEIRLOOM_ITEM_FLAG_SHOW_ONLY_IF_KNOWN = 0x01;
    private final int HEIRLOOM_ITEM_FLAG_PVP                = 0x02;

    private final int TOY_FLAG_NONE        = 0;
    private final int TOY_FLAG_FAVORITE    = 0x01;
    private final int TOY_FLAG_HAS_FANFARE = 0x02;


    private final byte MOUNT_STATUS_NONE   = 0x00;
    private final byte MOUNT_NEEDS_FANFARE = 0x01;
    private final byte MOUNT_IS_FAVORITE   = 0x02;


    private final WorldSession owner;

    private final HashMap<Integer, Integer> toys = new HashMap<>();

    private final HashMap<Integer, HeirloomData> heirlooms = new HashMap<Integer, HeirloomData>();

    private final HashMap<Integer, Byte> mounts = new HashMap<>();
    ...
    private final BitSet appearances = new BitSet();
    ...
    private final BNetAccountRepository bNetAccountRepo;
    private final AccountRepository accountRepo;

    public Collector(WorldSession owner) {
        this.owner = owner;
        this.characterRepo = owner.getWorldContext().getBean(CharacterRepository.class);
        this.bNetAccountRepo = owner.getWorldContext().getBean(BNetAccountRepository.class);
        this.accountRepo = owner.getWorldContext().getBean(AccountRepository.class);
        this.dbcObjectManager = owner.getWorldContext().getDbcObjectManager();
    }
    ...
    public final boolean addToy(int itemId, boolean isFavourite, boolean hasFanfare) {
        if (updateAccountToys(itemId, isFavourite, hasFanfare)) {
            if (owner.getPlayer() != null) {
                owner.getPlayer().addToy(itemId, getToyFlags(isFavourite, hasFanfare));
            }
    ...
    public final void saveAccountToys() {
        for (var pair : toys.entrySet()) {
            bNetAccountRepo.replaceAccountToys(owner.getBattleNetAccountId(), pair.getKey(),
                     (pair.getValue() & TOY_FLAG_FAVORITE) != 0,
                     (pair.getValue() & TOY_FLAG_HAS_FANFARE) != 0);
        }
    }
    ...
    public final void toySetFavorite(int itemId, boolean favorite) {
        ...
        if (favorite) {
            toys.put(itemId, toys.get(itemId) | TOY_FLAG_FAVORITE);
        } else {
            toys.put(itemId, toys.get(itemId) & ~TOY_FLAG_FAVORITE);
        }
    }
    ...
    public final void toyClearFanfare(int itemId) {
        ...
        toys.put(itemId, toys.get(itemId) & ~TOY_FLAG_HAS_FANFARE);
    }

    public final void onItemAdded(Item item) {
        var dbcObjectManager = owner.getWorldContext().getDbcObjectManager();
        if (dbcObjectManager.getHeirloomByItemId(item.getEntry()) != null) {
    ...
    public final void saveAccountHeirlooms() {
        for (var heirloom : heirlooms.entrySet()) {
            bNetAccountRepo.replaceAccountHeirlooms(owner.getBattleNetAccountId(),
                    heirloom.getKey(), heirloom.getValue().getHeirloomPlayerFlags());
        }
    }
    ...
    public final void loadHeirlooms() {
        for (var item : heirlooms.entrySet()) {
            owner.getPlayer().addHeirloom(item.getKey(), item.getValue().getHeirloomPlayerFlags());
        }
    }
    ...
    public final void addHeirloom(int itemId, int flags) {
        if (updateAccountHeirlooms(itemId, flags)) {
            owner.getPlayer().addHeirloom(itemId, flags);
        }
    }
    ...
    public final void upgradeHeirloom(int itemId, int castItem) {
        ...
        var heirloom = dbcObjectManager.getHeirloomByItemId(itemId);
        ...
        var flags = data.getHeirloomPlayerFlags();
        int bonusId = 0;
        var upgradeItemIds = heirloom.getUpgradeItemID();

        for (var upgradeLevel = 0; upgradeLevel < upgradeItemIds.length; ++upgradeLevel) {
            if (upgradeItemIds[upgradeLevel] == castItem) {
                flags |= 1 << upgradeLevel;
                bonusId = heirloom.getUpgradeItemBonusListID()[upgradeLevel];
            }
        }
        ...
        List<Integer> heirlooms = player.getActivePlayerData().getHeirlooms();
        var offset = heirlooms.indexOf(itemId);

        player.setHeirloomFlags(offset, (int) flags);
        data.setHeirloomPlayerFlags(flags);
    ...
    public final void checkHeirloomUpgrades(Item item) {
        ...
        var dbcObjectManager = owner.getWorldContext().getDbcObjectManager();
        var heirloom = dbcObjectManager.getHeirloomByItemId(item.getEntry());
        if (heirloom != null) {
            ...
            var heirloomItemId = heirloom.getStaticUpgradedItemID();
            int newItemId = 0;
            Heirloom heirloomDiff;
            while ((heirloomDiff = dbcObjectManager.getHeirloomByItemId(heirloomItemId)) != null) {
                if (player.getItemByEntry(heirloomDiff.getItemID()) != null) {
                    newItemId = heirloomDiff.getItemID();
                }
                var heirloomSub = dbcObjectManager.getHeirloomByItemId(heirloomDiff.getStaticUpgradedItemID());

                if (heirloomSub != null) {
                    heirloomItemId = heirloomSub.getItemID();
            ...
            if (!bonusListIDs.contains(data.getBonusId())) {
                item.addBonuses(data.getBonusId());
            }
        }
    }

    public final void loadMounts() {
        for (var m : mounts.entrySet()) {
            addMount(m.getKey(), m.getValue());
        }
    }


    public final void saveAccountMounts() {
        for (var mount : mounts.entrySet()) {
            bNetAccountRepo.replaceAccountMounts(owner.getBattleNetAccountId(), mount.getKey(), (byte) mount.getValue());
        }
    }


    public final boolean addMount(int spellId, byte flags, boolean factionMount) {
        return addMount(spellId, flags, factionMount, false);
    }

    public final boolean addMount(int spellId, byte flags) {
        return addMount(spellId, flags, false, false);
    }

    public final boolean addMount(int spellId, byte flags, boolean factionMount, boolean learned) {
        var player = owner.getPlayer();
        ...
        var dbcObjectManager = owner.getWorldContext().getDbcObjectManager();
        var mount = dbcObjectManager.getMount(spellId);
        ...
        var value = objectManager.getFactionSpecificMounts().get(spellId);
        ...
        if (mount.getPlayerConditionID() != 0) {

            if (PlayerConditions.isPlayerMeetingCondition(player, mount.getPlayerConditionID())) {
                return false;
            }
        }
    ...
    public final void mountSetFavorite(int spellId, boolean favorite) {
        ...
        if (favorite) {
            mounts.put(spellId, mounts.get(spellId) | MOUNT_IS_FAVORITE);
        } else {
            mounts.put(spellId, mounts.get(spellId).() & ~MountStatusFlags.isFavorite.());
        }
    ...
    public final void load() {
        var accountToys = bNetAccountRepo.findAccountToys(owner.getBattleNetAccountId());
        var battlePets = bNetAccountRepo.findBattlePets(owner.getBattleNetAccountId(), owner.getWorldContext().getWorldSettings().realmID);
        var battlePetSlots = bNetAccountRepo.findBattlePetSlots(owner.getBattleNetAccountId());
        var accountHeirlooms = bNetAccountRepo.findAccountHeirlooms(owner.getBattleNetAccountId());
        var accountMounts = bNetAccountRepo.findAccountMounts(owner.getBattleNetAccountId());
        var characterCounts = accountRepo.findCharacterCountsByAccountId(owner.getAccountId());
        var itemAppearances = bNetAccountRepo.findItemAppearances(owner.getBattleNetAccountId());
        var favoriteItemAppearances = bNetAccountRepo.selectFavoriteItemAppearances(owner.getBattleNetAccountId());
        var transmogIllusions = bNetAccountRepo.findTransmogIllusions(owner.getBattleNetAccountId());


        accountToys.forEach(battlenetAccountToy -> {
            toys.put(battlenetAccountToy.getItemId(),
                    getToyFlags(battlenetAccountToy.getIsFavourite(), battlenetAccountToy.getHasFanfare()));
        });

        accountHeirlooms.forEach(battlenetAccountHeirloom -> {
            var itemId = battlenetAccountHeirloom.getItemId();
            var flags = battlenetAccountHeirloom.getFlags();
            var heirloom = dbcObjectManager.getHeirloomByItemId(itemId);
            if (heirloom == null) {
                return;
            }
            ...
            for (var upgradeLevel = heirloom.getUpgradeItemID().length - 1; upgradeLevel >= 0; --upgradeLevel) {
                if ((flags & (1 << upgradeLevel)) != 0) {
                    bonusId = heirloom.getUpgradeItemBonusListID()[upgradeLevel];
                    break;
                }
            }
        ...
        accountMounts.forEach(battlenetAccountMount -> {
            var mount = dbcObjectManager.getMount(battlenetAccountMount.getMountSpellId());
            if(mount == null) {
                return;
            }
            mounts.put(battlenetAccountMount.getMountSpellId(), battlenetAccountMount.getFlags());
        });

        Short length = itemAppearances.stream().map(BattlenetItemAppearance::getBlobIndex)
                .max(Short::compareTo).orElse((short) 0);
        var blocks = new int[length];
        for (var battlenetItemAppearance : itemAppearances) {
            blocks[battlenetItemAppearance.getBlobIndex()] = battlenetItemAppearance.getAppearanceMask();
        }
        ...
        loadAccountTransmogIllusions(transmogIllusions);
    }
...
//		}
    ...
    private int getToyFlags(boolean isFavourite, boolean hasFanfare) {
        var flags = TOY_FLAG_NONE;
    ...
    private boolean updateAccountHeirlooms(int itemId, int flags) {
        if (heirlooms.containsKey(itemId)) {
            return false;
        }

        heirlooms.put(itemId, new HeirloomData(flags, 0));

        return true;
    }


    private void sendSingleMountUpdate(int spellId, int mountStatusFlags) {
        var player = owner.getPlayer();
